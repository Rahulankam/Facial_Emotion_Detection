...
facec = cv2.CascadeClassifier('model1/haarcascade_frontalface_default.xml')
...
selectbox = st.sidebar.selectbox(
    ...
    ("ferModel", "FacModel","Model3")
)


if selectbox == "ferModel":
    model = FacialExpressionModel('model1/model_a1.json','model1/model_weights1.h5')
    ...
    model = FacialExpressionModel('model2/emotiondetector.json','model2/emotiondetector.h5')
    
    Detect(imge.name)
elif selectbox == 'Model3':
    imge = st.file_uploader('Upload image',type=['JPEG','PNG','JPG'])
    st.image(imge)
    if imge is not None:
        file_img = {"FileName":imge.name,"Filetype":imge.type}
    ...
    def Detect(file_path):
        ...
        try:
            for (x,y,w,h) in faces:
                fc = gray_image[y:y+h,x:x+w]
                roi = cv2.resize(fc,(48,48))
                pred = EMOTIONS[np.argmax(model.predict(roi[np.newaxis,:,:,np.newaxis]))]
            print("Predicted Emotion is" + pred)
            st.write(pred)
        except:
            st.write('unable to detect')
    model = FacialExpressionModel('model3/emotion_detection_model_3.json','model3/emotion_detection_model_3.h5')
